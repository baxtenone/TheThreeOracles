...
import { OracleBall } from './components/OracleBall';
...
export default function App() {
  ...
  async function submitQuestion(event?: FormEvent) {
    ...
    inFlight.current = true; setBusy(true); setError(''); setAnswers(null);
  ...
  function signOut() { accessStorage.clear(); setAuthenticated(false); setAccessCode(''); setAnswers(null); setGenerated(null); setError(''); }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <nav className="mode-switch" aria-label="Choose an Oracle mode">
        ...
        <button className={view === 'pose' ? 'active' : ''} onClick={() => { setView('pose'); setError(''); }}>Pose a Question</button>
      </nav>

      {view === 'ask' && <>
        <form className="ask-panel" onSubmit={submitQuestion}>
          <label htmlFor="oracle-question">What do you want to know?</label>
          <textarea id="oracle-question" value={question} onChange={(event) => setQuestion(event.target.value)} maxLength={MAX_QUESTION_LENGTH + 1} placeholder="Should humans colonize Mars?" rows={3} disabled={busy}/>
          <div className="ask-panel__footer"><span className={MAX_QUESTION_LENGTH - question.length < 50 ? 'count count--low' : 'count'}>{MAX_QUESTION_LENGTH - question.length}</span><button className="primary-button" disabled={busy || !question.trim()}>{busy ? 'Consulting…' : 'Ask the Oracles'} <span aria-hidden="true">→</span></button></div>
        </form>
        <section className="oracle-row" aria-busy={busy} aria-live="polite">{PEOPLE.map((person) => <OracleBall key={person} name={person} response={answerByPerson.get(person)} loading={busy}/>)}</section>
      ...
      {view === 'pose' && <section className="pose-panel">
        ...
        <div className="pose-actions"><button className="primary-button" onClick={poseQuestion} disabled={busy}>{generated ? 'Another Question' : 'Pose a Question'} <span>↻</span></button>{generated && <button className="secondary-button" onClick={askGenerated}>Ask the Oracles <span>→</span></button>}</div>
      </section>}

      {view === 'settings' && <Settings onClear={() => { questionHistory.clear(); setError('Question history cleared.'); }} onSignOut={signOut}/>}
      {error && <div className="notice" role="alert">{error}</div>}
    </main>
    <footer>BRASADA RANCH · V1.0</footer>
  </div>;
}
...
function AccessGate({ code, setCode, submit, busy, error }: { code: string; setCode: (value: string) => void; submit: (event: FormEvent<HTMLFormElement>) => void; busy: boolean; error: string }) {
  return <main className="gate"><div className="gate__orbits" aria-hidden="true"><i/><i/><i/></div><form className="gate__card" onSubmit={submit}>
    <span className="gate__mark">◈</span><p className="eyebrow">A PRIVATE TABLE</p><h1>The Three<br/><em>Oracles</em></h1><p>Bruce, Kevin, and Travis have the code.</p>
...
function GroupResult({ result }: { result: AnswerResult['group'] }) {
  ...
  return <section className="group-result"><p className="eyebrow">THE TABLE SAYS</p><h2>{labels[result.alignment]}</h2><p>{result.summary}</p>{result.mainDisagreement && <p><strong>The rub:</strong> {result.mainDisagreement}</p>}{result.conversationStarter && <div className="starter"><span>WORTH ARGUING ABOUT</span><p>{result.conversationStarter}</p></div>}</section>;
}
...
function Settings({ onClear, onSignOut }: { onClear: () => void; onSignOut: () => void }) {
  return <section className="settings-panel"><p className="eyebrow">SETTINGS</p><h2>Keep it simple.</h2><div className="settings-list">
    <div><h3>The profiles</h3><p>Bruce thinks in systems, Kevin tests the evidence, and Travis finds the assumption underneath. Full source profiles live in <code>src/config/oracleProfiles.ts</code>.</p></div>
    <div><h3>Question history</h3><p>Recent generated questions stay only in this browser and help avoid repeats.</p><button className="text-button" onClick={onClear}>Clear Question History</button></div>
  ...
  </div><p className="version">THE THREE ORACLES · VERSION 1.0</p></section>;
}

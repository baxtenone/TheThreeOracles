...
export const MysticBubble = forwardRef<HTMLButtonElement, { response: Response; onClose: () => void; onCloudTap?: () => void }>(function MysticBubble({ response, onClose, onCloudTap }, closeRef) {
  return (
    <section id="oracle-answer-cloud" className={`mystic-bubble mystic-bubble--${accents[response.person]} ${response.mode === 'dodge' ? 'mystic-bubble--dodge' : ''}`} data-oracle-bubble role="dialog" aria-modal="false" aria-labelledby="oracle-answer-title" onClick={(event) => {
      const target = event.target as Element;
      if (!target.closest('button, summary, details')) onCloudTap?.();
    }}>
